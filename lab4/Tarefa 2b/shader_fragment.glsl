...
void main()
{
    ...
    // Variáveis da spotlight
    float spotlight_angle = 3.141592f / 6.0f; // 30°
    vec4 spotlight_pos    = vec4(0.0,2.0,1.0,0.0);
    // vec4 spotlight_dir    = spotlight_pos - vec4(0.0,-1.0,0.0,0.0);
    vec4 spotlight_dir    = vec4(0.0,-1.0,0.0,0.0);

    // Vetor que define o sentido da fonte de luz em relação ao ponto atual.
    // vec4 l = normalize(vec4(1.0,1.0,0.5,0.0));
    // vec4 l = normalize(camera_position - p);
    vec4 l = normalize(spotlight_pos-p);
    ...
    vec3 phong_specular_term  = Ks * I * pow(max_phong, q); // PREENCHA AQUI o termo especular de Phong

    // Cor final do fragmento calculada com uma combinação dos termos difuso,
    // especular, e ambiente. Veja slide 134 do documento "Aula_17_e_18_Modelos_de_Iluminacao.pdf".
    if (dot(-l, spotlight_dir) < cos(spotlight_angle)) {
    // if (false) {
      color = ambient_term;
    } else {
